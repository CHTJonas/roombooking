...
class TwoFactorController < ApplicationController
  ...
  def create
    ...
    if auth
      ...
      log_abuse "#{current_user.name} successfully completed two-factor authentication"
      Roombooking::InfoCounter.poke('Successful 2FAs since boot')
      alert = { 'class' => 'success', 'message' => 'You have successfully logged in.' }
      flash[:alert] = alert
      redirect_to url
    else
      log_abuse "#{current_user.name} attempted two-factor authentication, but failed"
