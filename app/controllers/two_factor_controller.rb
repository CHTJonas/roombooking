...
class TwoFactorController < ApplicationController
  ...
  def create
    ...
    code = params[:totp]
    token = login_user.two_factor_token
    auth = token.verify(code)
    if auth
      session[:tfa] = auth
      log_abuse "#{login_user.to_log_s} successfully completed two-factor authentication"
      Roombooking::InfoCounter.poke('Successful 2FAs since boot')
      alert = { 'class' => 'success', 'message' => 'You have successfully logged in.' }
      flash[:alert] = alert
      redirect_to url
    else
      log_abuse "#{login_user.to_log_s} attempted two-factor authentication, but failed"
